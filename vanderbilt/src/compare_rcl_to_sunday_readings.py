...
def normalize_rcl_key(lit_name):
    ...
    if lit_name.startswith("Liturgy of the Passion") or lit_name.startswith(
        "Palm Sunday"
    ):
        return "Palm Sunday"
...
def parse_reading(reading):
    ...
    for alt in alternates:
        ...
        if m:
            ...
            parsed.append(
                {"book": book, "chapter": chapter, "verses": verse_set, "raw": alt}
            )
        else:
            ...
            if r["book"] == i["book"]:
                if r["chapter"] == i["chapter"]:
                    if r["verses"] and i["verses"]:
                        overlap = len(r["verses"] & i["verses"]) / max(
                            1, len(r["verses"] | i["verses"])
                        )
                        score = overlap
                        if overlap == 1.0:
                            category = "Exact"
...
def compare_readings(rcl, internal):
    ...
    for week, rcl_readings in rcl.items():
        ...
        for i, label in enumerate(["OT", "Epistle", "Gospel", "Psalm"]):
            ...
            internal_val = (
                internal[week][i]
                if week in internal and len(internal[week]) > i
                else ""
            )
            percent, best_alt, category, details = best_reading_match_with_alternates(
                rcl_val, internal_val
            )
    ...
    if rcl_key.startswith("Nativity of the Lord - Proper") and rcl is not None:
        ...
        for proper in propers:
            ...
            if rcl_readings and len(rcl_readings) == 4:
                score = sum(
                    best_reading_match(a, b)[0]
                    for a, b in zip(rcl_readings, christmas_readings)
                )
                if score > best_score:
                    best_score = score
...
def main():
    ...
    html_lines = []
    html_lines.append(
        '<html><head><meta charset="utf-8"><title>RCL Comparison Report</title>'
    )
    html_lines.append(
        "<style>body{font-family:sans-serif;} table{border-collapse:collapse;margin-bottom:1em;} th,td{border:1px solid #ccc;padding:4px;} summary{font-weight:bold;} details{margin-bottom:1em;} .summary-table td,.summary-table th{border:none;}"
    )
    html_lines.append("summary.year-summary { font-size: 1.3em; margin-left: 0; }")
    html_lines.append(
        "summary.category-summary { font-size: 1.1em; margin-left: 1em; }"
    )
    html_lines.append(
        "summary.subcategory-summary { font-size: 1em; margin-left: 2em; }"
    )
    ...
    for year, csvfile in RCL_FILES.items():
        ...
        print(
            f"  Internal weeks: {sorted([k for k in sunday_readings if year in sunday_readings[k]])}"
        )
        ...
        missing = [
            (week, diffs[week]["missing_in_internal"])
            for week in diffs
            if "missing_in_internal" in diffs[week]
        ]
        if missing:
        ...
        for cat in sorted(mismatch_categories.keys()):
            ...
            for m in mismatch_categories[cat]:
                human_summary.append(
                    f"      {m['week']} [{m['label']}]: {m['expected']} vs {m['found']} (match: {m['match_percent']}%) {m['details']}"
                )
        human_summary.append("")
        per_year_html.append(
            f'<details open><summary class="year-summary">Year {html.escape(year)}</summary>'
        )
        missing = [
            (week, diffs[week]["missing_in_internal"])
            for week in diffs
        ...
        if missing:
            per_year_html.append(
                '<details><summary class="category-summary">Missing in Internal</summary>'
            )
            per_year_html.append("<table><tr><th>Week</th><th>Readings</th></tr>")
            for week, readings in missing:
                per_year_html.append(
                    "<tr><td>{}</td><td>{}</td></tr>".format(
                        html.escape(week), html.escape(str(readings))
                    )
                )
        ...
        per_year_html.append(
            '<details><summary class="category-summary">Mismatches by Category</summary>'
        )
        for cat in sorted(mismatch_categories.keys()):
            count = len(mismatch_categories[cat])
            per_year_html.append(
                f'<details><summary class="subcategory-summary">{html.escape(cat)} ({count})</summary>'
            )
            per_year_html.append(
                "<table><tr><th>Week</th><th>Label</th><th>Expected</th><th>Found</th><th>Match %</th><th>Details</th></tr>"
            )
            for m in mismatch_categories[cat]:
                expected_display = html.escape(m["expected"])
                if m.get("best_alt") and m["best_alt"] != m["expected"]:
                    expected_display += (
                        f'<br/><em>(best match: {html.escape(m["best_alt"])})</em>'
                    )
                per_year_html.append(
    ...
    html_lines.append(
        f"<tr><th>Total readings compared</th><td>{total_readings}</td></tr>"
    )
    ...
    html_lines.append('<table class="summary-table">')
    html_lines.append(
        "<tr><th>Category</th><th>Count</th><th>Percent of Total</th></tr>"
    )
    for cat in sorted(all_category_counts.keys()):
        percent = (
            (all_category_counts[cat] / total_readings * 100) if total_readings else 0
        )
        html_lines.append(
            f"<tr><td>{html.escape(cat)}</td><td>{all_category_counts[cat]}</td><td>{percent:.1f}%</td></tr>"
        )
    html_lines.append("</table>")
