...
FEASTS_RCL_TO_INTERNAL = {
    ...
    'Presentation of the Lord': ('christmas', '02-02', 'Presentation of Christ at the Temple'),
    'Annunciation of the Lord': ('christmas', '03-25', 'The Annunciation of our Lord'),
    'Holy Name of Jesus': ('christmas', '01-01', 'The Naming and Circumcision of Jesus'),
    ...
    'Visitation of Mary to Elizabeth': ('christmas', '05-31', 'The Visit of the Blessed Virgin Mary to Elizabeth'),
...
def normalize_rcl_key(lit_name):
    ...
    if lit_name.startswith("Trinity Sunday"):
        return "Trinity"
    if lit_name.startswith("Palm Sunday") or lit_name.startswith("Liturgy of the Passion"):
        return "Palm Sunday"
...
def main():
    ...
    for year, csvfile in RCL_FILES.items():
        ...
        for rcl_key, readings in rcl.items():
            ...
            if internal_key in [
                ...
                'Epiphany 1','Epiphany 2','Epiphany 3','Epiphany 4',
                '1 before Lent',
    ...
    with open(feasts_path, 'w', encoding='utf-8') as f:
        f.write(feasts_py)
        pprint.pprint({k: dict(v) for k,v in feast_readings.items()}, stream=f, width=120)
        f.write('\n')
    print(f"Wrote {len(sunday_readings)} Sundays and {sum(len(v) for v in feast_readings.values())} feasts.")
