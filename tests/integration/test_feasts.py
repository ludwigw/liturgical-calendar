...
from liturgical_calendar.data.feasts_data import get_liturgical_feast
...
class TestLookupFeast(unittest.TestCase):
    ...
    def test_christmas_feasts(self):
        self.assertEqual(
            get_liturgical_feast("christmas", "01-01")["name"],
            "The Naming and Circumcision of Jesus",
        )
        self.assertEqual(
            get_liturgical_feast("christmas", "12-25")["name"], "Christmas"
        )
        self.assertEqual(get_liturgical_feast("christmas", "01-06")["name"], "Epiphany")

    def test_feast_types(self):
        self.assertEqual(
            get_liturgical_feast("easter", -46)["type"], "Principal Holy Day"
        )
        self.assertEqual(get_liturgical_feast("easter", 0)["type"], "Principal Feast")
        self.assertEqual(
            get_liturgical_feast("christmas", "01-06")["type"], "Principal Feast"
        )

    def test_feast_urls(self):
        self.assertEqual(
            get_liturgical_feast("easter", -46)["url"],
            "https://en.wikipedia.org/wiki/Ash_Wednesday",
        )
        self.assertEqual(
            get_liturgical_feast("christmas", "12-25")["url"],
    ...
    def test_sunday_cycle(self):
        ...
        for date, cycle in sundays:
            ...
            self.assertIn(
                expected,
                readings,
                f"Expected reading '{expected}' not found for {date} (cycle {cycle})",
            )
...
class TestWeekCalculation(unittest.TestCase):
    def test_specific_week_calculations(self):
        ...
        test_cases = [
            (
                "2025-02-26",
                "2 before Lent",
                "Should be 2 before Lent, not 1 before Lent",
            ),
    ...
    def test_easter_week_calculations(self):
        ...
        self.assertEqual(info.get("week"), "Easter 6", "25 May 2025 should be Easter 6")

        # 30 April 2026 should be Easter 4
        info = liturgical_calendar("2026-04-30")
        self.assertEqual(
            info.get("week"), "Easter 4", "30 April 2026 should be Easter 4"
        )
    ...
    def test_christmas_epiphany_weeks(self):
        ...
        self.assertEqual(
            info.get("week"), "Christmas 2", "5 Jan 2026 should be Christmas 2"
        )

        # 20 January 2026 should be Epiphany 2
        info = liturgical_calendar("2026-01-20")
        self.assertEqual(
            info.get("week"), "Epiphany 2", "20 January 2026 should be Epiphany 2"
        )
    ...
    def test_holy_week_calculation(self):
        ...
        self.assertEqual(
            info.get("week"), "Holy Week", "31 March 2026 should be Holy Week"
        )
        self.assertEqual(
            info.get("season"),
            "Holy Week",
    ...
    def test_proper_week_calculation(self):
        ...
        self.assertEqual(
            info.get("week"), "Proper 7", "22 June 2025 should be Proper 7"
        )
    ...
    def test_lent_weekday_calculation(self):
        ...
        self.assertEqual(
            info.get("season"), "Lent", "20 Feb 2026 should be in Lent season"
        )
    ...
    def test_lent_week_1_calculation(self):
        ...
        for date_str in lent_1_dates:
            with self.subTest(date=date_str):
                info = liturgical_calendar(date_str)
                self.assertEqual(
                    info.get("week"), "Lent 1", f"{date_str} should be Lent 1"
                )
                self.assertEqual(
                    info.get("season"), "Lent", f"{date_str} should be in Lent season"
                )
    ...
    def test_lent_week_2_calculation(self):
        ...
        for date_str in lent_2_dates:
            with self.subTest(date=date_str):
                info = liturgical_calendar(date_str)
                self.assertEqual(
                    info.get("week"), "Lent 2", f"{date_str} should be Lent 2"
                )
                self.assertEqual(
                    info.get("season"), "Lent", f"{date_str} should be in Lent season"
                )
    ...
    def test_pentecost_and_trinity(self):
        ...
        self.assertEqual(
            info.get("week"), "Pentecost", "30 May 2026 should be Pentecost"
        )
        self.assertEqual(
            info.get("season"), "Pentecost", "30 May 2026 should be in Pentecost season"
        )

        # 6 June 2026 should be Trinity
        info = liturgical_calendar("2026-06-06")
        self.assertEqual(info.get("week"), "Trinity", "6 June 2026 should be Trinity")
        self.assertEqual(
            info.get("season"), "Trinity", "6 June 2026 should be in Trinity season"
        )
    ...
    def test_trinity_2025(self):
        ...
        self.assertEqual(
            info.get("season"), "Trinity", "15 June 2025 should be in Trinity season"
        )
        ...
        self.assertEqual(
            info.get("season"), "Trinity", "22 June 2025 should be in Trinity season"
        )
    ...
    def test_season_transitions(self):
        ...
        self.assertEqual(
            info.get("season"), "Advent", "24 Dec 2026 should be in Advent season"
        )

        info = liturgical_calendar("2026-12-25")  # Christmas Day
        self.assertEqual(
            info.get("season"), "Christmas", "25 Dec 2026 should be in Christmas season"
        )

        # Test Christmas to Epiphany transition
        info = liturgical_calendar("2026-01-05")  # 12th Day of Christmas
        self.assertEqual(
            info.get("season"), "Christmas", "5 Jan 2026 should be in Christmas season"
        )

        info = liturgical_calendar("2026-01-06")  # Epiphany
        self.assertEqual(
            info.get("season"), "Epiphany", "6 Jan 2026 should be in Epiphany season"
        )

        # Test Epiphany to Ordinary Time transition (before Pre-Lent)
        info = liturgical_calendar("2026-02-01")  # Last day of Epiphany
        self.assertEqual(
            info.get("season"), "Epiphany", "1 Feb 2026 should be in Epiphany season"
        )
        ...
        self.assertEqual(
            info.get("season"),
            "Ordinary Time",  # After render_week_name
            "10 Feb 2026 should be in Ordinary Time season (after render_week_name)",
        )
        self.assertEqual(
            info.get("week"), "2 before Lent", "10 Feb 2026 should be '2 before Lent'"
        )
        ...
        self.assertEqual(
            info.get("season"), "Lent", "18 Feb 2026 should be in Lent season"
        )

        # Test Lent to Holy Week transition
        info = liturgical_calendar("2026-03-28")  # Last day of Lent
        self.assertEqual(
            info.get("season"), "Lent", "28 Mar 2026 should be in Lent season"
        )

        info = liturgical_calendar("2026-03-29")  # Palm Sunday
        self.assertEqual(
            info.get("season"), "Holy Week", "29 Mar 2026 should be in Holy Week season"
        )

        # Test Holy Week to Easter transition
        info = liturgical_calendar("2026-04-04")  # Last day of Holy Week
        self.assertEqual(
            info.get("season"), "Holy Week", "4 Apr 2026 should be in Holy Week season"
        )

        info = liturgical_calendar("2026-04-05")  # Easter Sunday
        self.assertEqual(
            info.get("season"), "Easter", "5 Apr 2026 should be in Easter season"
        )

        # Test Easter to Pentecost transition
        info = liturgical_calendar("2026-05-23")  # Last day of Easter
        self.assertEqual(
            info.get("season"), "Easter", "23 May 2026 should be in Easter season"
        )

        info = liturgical_calendar("2026-05-24")  # Pentecost
        self.assertEqual(
            info.get("season"), "Pentecost", "24 May 2026 should be in Pentecost season"
        )
        ...
        self.assertEqual(
            info.get("season"), "Trinity", "31 May 2026 should be in Trinity season"
        )
    ...
    def test_edge_cases_multiple_years(self):
        ...
        self.assertIsNotNone(info.get("season"), "Leap day should have a valid season")

        # Test year boundary transitions
        info = liturgical_calendar("2025-12-31")  # New Year's Eve
        self.assertIsNotNone(
            info.get("season"), "New Year's Eve should have a valid season"
        )

        info = liturgical_calendar("2026-01-01")  # New Year's Day
        self.assertIsNotNone(
            info.get("season"), "New Year's Day should have a valid season"
        )
        ...
        self.assertEqual(
            info.get("season"), "Easter", "31 Mar 2024 should be in Easter season"
        )

        # 2025: Easter on April 20 (late)
        info = liturgical_calendar("2025-04-20")
        self.assertEqual(
            info.get("season"), "Easter", "20 Apr 2025 should be in Easter season"
        )

        # Test Pre-Advent (late in the year)
        info = liturgical_calendar("2026-11-20")  # Late November
        self.assertIsNotNone(
            info.get("season"), "Late November should have a valid season"
        )
    ...
    def test_week_number_edge_cases(self):
        ...
        self.assertEqual(info.get("week"), "Advent 1", "30 Nov 2026 should be Advent 1")

        info = liturgical_calendar("2026-12-25")  # Christmas 1 (Christmas Day)
        self.assertEqual(info.get("week"), "Advent 4", "25 Dec 2026 should be Advent 4")

        info = liturgical_calendar("2026-01-06")  # Epiphany 1
        self.assertEqual(
            info.get("week"), "Christmas 2", "6 Jan 2026 should be Christmas 2"
        )
    ...
    def test_additional_edge_cases(self):
        ...
        info = liturgical_calendar("2026-01-01")  # New Year's Day
        self.assertIsNotNone(
            info.get("season"), "1 Jan 2026 should have a valid season"
        )

        info = liturgical_calendar("2026-12-31")  # New Year's Eve
        self.assertIsNotNone(
            info.get("season"), "31 Dec 2026 should have a valid season"
        )

        # Test leap year edge case
        info = liturgical_calendar("2024-02-29")  # Leap day
        self.assertIsNotNone(
            info.get("season"), "29 Feb 2024 should have a valid season"
        )
        ...
        info = liturgical_calendar("2026-11-20")  # Late November
        self.assertIsNotNone(
            info.get("season"), "20 Nov 2026 should have a valid season"
        )
        ...
        self.assertEqual(
            info.get("season"), "Christmas", "26 Dec 2026 should be in Christmas season"
        )

        info = liturgical_calendar("2026-12-27")  # Sunday after Christmas
        self.assertEqual(
            info.get("season"), "Christmas", "27 Dec 2026 should be in Christmas season"
        )
    ...
    def test_weekday_vs_sunday_calculations(self):
        ...
        self.assertEqual(
            info.get("week"), "Epiphany 4", "1 Feb 2026 (Sunday) should be Epiphany 4"
        )

        info = liturgical_calendar("2026-02-02")  # Monday
        self.assertEqual(
            info.get("week"), "Epiphany 4", "2 Feb 2026 (Monday) should be Epiphany 4"
        )

        info = liturgical_calendar("2026-02-03")  # Tuesday
        self.assertEqual(
            info.get("week"), "Epiphany 4", "3 Feb 2026 (Tuesday) should be Epiphany 4"
        )
        ...
        self.assertEqual(
            info.get("week"), "Lent 1", "22 Feb 2026 (Sunday) should be Lent 1"
        )

        info = liturgical_calendar("2026-02-23")  # Monday
        self.assertEqual(
            info.get("week"), "Lent 1", "23 Feb 2026 (Monday) should be Lent 1"
        )

        info = liturgical_calendar("2026-02-24")  # Tuesday
        self.assertEqual(
            info.get("week"), "Lent 1", "24 Feb 2026 (Tuesday) should be Lent 1"
        )
    ...
    def test_proper_weeks_first_and_last(self):
        ...
        self.assertEqual(info.get("week"), "Proper 7", "22 Jun 2025 should be Proper 7")
        ...
        self.assertTrue(
            "Proper" in info.get("week")
            or "Trinity" in info.get("week")
            or "before Advent" in info.get("week"),
    ...
    def test_advent_sunday_and_transition(self):
        ...
        self.assertTrue(
            info.get("season") in ["Trinity", "Ordinary Time", "Pre-Advent"],
            "2 Dec 2023 should be in Trinity/Ordinary/Pre-Advent season",
        )
        info = liturgical_calendar("2023-12-03")  # Advent Sunday
        self.assertEqual(
            info.get("season"), "Advent", "3 Dec 2023 should be in Advent season"
        )
    ...
    def test_christmas_eve_and_christmas_day_on_sunday(self):
        ...
        self.assertEqual(
            info.get("week"), "Advent 4", "18 Dec 2016 should be Advent 4 (week name)"
        )
        info = liturgical_calendar("2016-12-24")  # Christmas Eve (Saturday)
        self.assertEqual(
            info.get("season"), "Advent", "24 Dec 2016 should be in Advent season"
        )
        info = liturgical_calendar("2016-12-25")  # Christmas Day (Sunday)
        self.assertEqual(
            info.get("season"), "Christmas", "25 Dec 2016 should be in Christmas season"
        )
        self.assertEqual(
            info.get("week"),
            "Christmas 1",
    ...
    def test_epiphany_on_sunday_vs_weekday(self):
        ...
        self.assertEqual(
            info.get("season"), "Epiphany", "6 Jan 2017 should be in Epiphany season"
        )
        # 2012: Epiphany is Friday, but check the following Sunday
        info = liturgical_calendar("2012-01-08")  # Sunday after Epiphany
        self.assertEqual(
            info.get("season"), "Epiphany", "8 Jan 2012 should be in Epiphany season"
        )
        self.assertTrue(
            "Epiphany" in info.get("week"), "8 Jan 2012 should be an Epiphany week"
        )
        # 2011: Epiphany is Thursday, Sunday after is Jan 9
        info = liturgical_calendar("2011-01-09")
        self.assertEqual(
            info.get("season"), "Epiphany", "9 Jan 2011 should be in Epiphany season"
        )
        self.assertTrue(
            "Epiphany" in info.get("week"), "9 Jan 2011 should be an Epiphany week"
        )
    ...
    def test_multi_year_robustness(self):
        ...
        self.assertEqual(
            info.get("season"), "Easter", "23 Mar 2008 should be in Easter season"
        )
        # Late Easter: 2011 (April 24)
        info = liturgical_calendar("2011-04-24")  # Easter
        self.assertEqual(
            info.get("season"), "Easter", "24 Apr 2011 should be in Easter season"
        )
        # Advent 4 on Christmas Eve: 2017
        info = liturgical_calendar("2017-12-24")  # Advent 4 and Christmas Eve
        self.assertEqual(
            info.get("season"), "Advent", "24 Dec 2017 should be in Advent season"
        )
        self.assertEqual(
            info.get("week"), "Advent 4", "24 Dec 2017 should be Advent 4 (week name)"
        )
...
class TestReadingsCoverage(unittest.TestCase):
    ...
    def test_sunday_cycle_coverage(self):
        ...
        for date_str, expected_cycle in sunday_tests:
            with self.subTest(date=date_str, cycle=expected_cycle):
                ...
                self.assertTrue(
                    readings, f"No readings for {date_str} ({expected_cycle} cycle)"
                )
    ...
    def test_weekday_cycle_coverage(self):
        ...
        for date_str, expected_cycle in weekday_tests:
            with self.subTest(date=date_str, cycle=expected_cycle):
                ...
                if (info.get("name") in principal_feasts) or (
                    date_str[5:] in {"12-25", "01-01", "01-06"}
                ):
                    continue
                self.assertTrue(
                    readings,
                    f"No readings for {date_str} (weekday cycle {expected_cycle})",
                )
                from liturgical_calendar.funcs import day_of_week
                ...
                day_of_week_val = day_of_week(year, month, day)
                self.assertNotEqual(
                    day_of_week_val, 0, f"{date_str} should be a weekday"
                )
    ...
    def test_season_reading_coverage(self):
        ...
        for date_str, season_name in season_tests:
            with self.subTest(date=date_str, season=season_name):
                ...
                readings = info.get("readings", [])
                self.assertTrue(
                    readings, f"No readings for {date_str} ({season_name} season)"
                )

                # Check that the season matches
                actual_season = info.get("season", "")
                self.assertTrue(
                    season_name.lower() in actual_season.lower()
                    or (season_name == "Ordinary" and "Trinity" in actual_season),
    ...
    def test_feast_day_readings(self):
        ...
        for date_str, feast_name in feast_tests:
            with self.subTest(date=date_str, feast=feast_name):
                ...
                self.assertTrue(
                    feast_name.lower() in actual_name.lower(),
    ...
    def test_weekday_vs_sunday_reading_differences(self):
        ...
        sunday_readings = readings_manager.get_readings_for_date(
            sunday_date, sunday_info
        )
        monday_readings = readings_manager.get_readings_for_date(
            monday_date, monday_info
        )
