...
class ImageGenerationPipeline:
    ...
    def _create_layout(self, data):
        # Prepare fonts
        serif_font_36 = self.font_manager.get_font('HappyTimes-Regular.otf', self.header_font_size)
        serif_font_96 = self.font_manager.get_font('HappyTimes-Regular.otf', self.title_font_size)
        sans_font_36 = self.font_manager.get_font('HankenGrotesk-Medium.ttf', self.column_font_size)
        sans_font_36_uc = self.font_manager.get_font('HankenGrotesk-Medium.ttf', self.column_font_size)
        fonts = {
            'serif': serif_font_36,
        ...
        header_layout = self.layout_engine.create_header_layout(season, date_text, {'serif': serif_font_36, 'sans_uc': sans_font_36_uc}, None, self.width, self.padding, font_manager=self.font_manager)
        ...
        artwork_layout = self.layout_engine.create_artwork_layout(
            data['artwork'], data['next_artwork'], self.width, self.artwork_size, art_y, fonts={
                'serif': serif_font_36,
                'sans': sans_font_36,
                'sans_32': fonts['sans_32'],
                'sans_26': fonts['sans_26'],
            }, draw=None, font_manager=self.font_manager
        )
        # Title
        ...
        title_layout = self.layout_engine.create_title_layout(
            title, {'serif_96': serif_font_96}, None, self.width, self.padding, self.title_font_size, self.title_line_height, title_y, font_manager=self.font_manager
        )
        ...
        readings_layout = self.layout_engine.create_readings_layout(
            week, readings, {'serif': serif_font_36, 'sans_uc': sans_font_36_uc}, None, self.width, self.padding, col_y, 48, font_manager=self.font_manager
        )
        layout = {
    ...
    def _save_image(self, img, date_str, layout, fonts, data, out_path=None):
        ...
        self.builder.build_image(date_str, data['info'], data['artwork'], layout, fonts, out_path)
        return out_path 